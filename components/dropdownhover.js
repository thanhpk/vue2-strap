...
var pluginName = 'bootstrapDropdownHover',
		defaults = {
			...
			// 'disable' disables dropdown toggling with clicking when mouse
			//           is detected
			//	'sticky' means if we click on an opened dropdown then it will
			//           not hide on mouseleave but on a second click only
			hideTimeout: 200
		},
		_hideTimeoutHandler,
		_hardOpened = false,
...
function bindEvents(dropdown) {
	...
	$('.dropdown-toggle, .dropdown-menu', dropdown.element.parent())
...
function removeEvents(dropdown) {
	$('.dropdown-toggle, .dropdown-menu', dropdown.element.parent())
		.off('.dropdownhover');
	// seems that bootstrap binds the click handler twice after we
	// reinitializing the plugin after a destroy...
	$('.dropdown-toggle, .dropdown-menu', dropdown.element.parent())
		.off('.dropdown');
